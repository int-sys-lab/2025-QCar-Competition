...
class LaneKeepingThread(Thread):
    def __init__(self, camera_interface):
        ...
        self.display_window = "Lane Detection View"
        
        # Color thresholds
        self.yellow_lower = np.array([50, 50, 80])  # Yellow lane
        self.yellow_upper = np.array([70, 255, 255])
        self.curb_lower = np.array([0, 0, 200])       # Curb (gray/white)
        self.curb_upper = np.array([180, 50, 255])
        ...
        # ROI parameters
        self.roi_height = 0.2  # Use bottom 20% of image
        self.roi_width = 0.2   # Use middle 20% of image
    ...
    def run(self):
        ...
        while self.running:
            ...
            if self.camera_interface.d435Color.read_RGB():
                frame = self.camera_interface.d435Color.imageBufferRGB.copy()
                
                if frame is not None:
                    # Process the frame
                    steering, processed_frame = self.process_frame(frame)
                    
                    # Update steering adjustment
                    with self.lock:
                        self.steering_adjustment = steering
                    
                    # Display the processed frame
                    cv2.imshow(self.display_window, processed_frame)
                    cv2.waitKey(1)
    ...
    def process_frame(self, frame):
        ...
        # Left ROI - detect both yellow lane and curb (mutually exclusive)
        left_roi = hsv[roi_top:height, :width//2]
        
        # First try to detect yellow lane
        yellow_mask = cv2.inRange(left_roi, self.yellow_lower, self.yellow_upper)
        lane_pos = self.find_feature_position(yellow_mask)
        
        # If no yellow lane detected, try to detect curb
        if lane_pos is None:
            curb_mask = cv2.inRange(left_roi, self.curb_lower, self.curb_upper)
            left_curb_pos = self.find_feature_position(curb_mask)
        else:
            left_curb_pos = None
        
        # Right ROI - only detect curb
        right_roi = hsv[roi_top:height, width//2:]
        curb_mask = cv2.inRange(right_roi, self.curb_lower, self.curb_upper)
        right_curb_pos = self.find_feature_position(curb_mask)
        if right_curb_pos is not None:
            right_curb_pos += width//2  # Adjust for ROI offset
        
        # Calculate steering using both sides
        steering = self.calculate_steering(
            lane_pos if lane_pos is not None else left_curb_pos,
            right_curb_pos,
            width
        )
        ...
        # Draw ROIs
        cv2.rectangle(processed_frame, (0, roi_top), (width//2, height), (0, 255, 0), 2)  # Left
        cv2.rectangle(processed_frame, (width//2, roi_top), (width, height), (0, 0, 255), 2)  # Right
        
        # Draw detected features
        if lane_pos is not None:
            cv2.circle(processed_frame, (lane_pos, (roi_top + height)//2), 10, (0, 255, 255), -1)  # Yellow
        elif left_curb_pos is not None:
            cv2.circle(processed_frame, (left_curb_pos, (roi_top + height)//2), 10, (255, 0, 255), -1)  # Left curb (magenta)
        
        if right_curb_pos is not None:
            cv2.circle(processed_frame, (right_curb_pos, (roi_top + height)//2), 10, (255, 0, 0), -1)  # Right curb (blue)
        
        # Draw steering info
        cv2.putText(processed_frame, f"Steering: {steering:.3f}", (20, 30), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        
        return steering, processed_frame

    def find_feature_position(self, mask):
        vertical_sum = np.sum(mask, axis=0)
        peaks, _ = find_peaks(vertical_sum, height=1000, distance=50)
        return peaks[np.argmax(vertical_sum[peaks])] if len(peaks) > 0 else None
    
    def calculate_steering(self, left_pos, right_pos, image_width):
        if left_pos is not None and right_pos is not None:
            # Center between left and right features
            center = (left_pos + right_pos) / 2
            error = center - image_width / 2
            steering = -error * self.steering_gain
        elif left_pos is not None:
            # Only left feature detected
            if left_pos < image_width / 2:  # Left lane/curb
                error = left_pos - (image_width / 4)  # Target 1/4 from left
            else:  # Shouldn't happen since we're only looking at left half
                error = left_pos - (image_width / 2)
            steering = -error * self.steering_gain * 0.7
        elif right_pos is not None:
            # Only right curb detected
            error = right_pos - (3 * image_width / 4)  # Target 3/4 from left
            steering = -error * self.steering_gain * 0.7
        else:
            # Nothing detected
            steering = 0
        
        return np.clip(steering, -self.max_adjustment, self.max_adjustment)
...
def controlLoop():
    ...
    # Initialize
    # Initialize lane keeping thread
    lane_keeper = LaneKeepingThread(cameraInterfacingLab)
    lane_keeper.start()
    ...
    with qcar, gps:
        ...
        while (t < tf+startDelay) and (not KILL_THREAD):
            ...
            if t < startDelay:
                ...
                #Stop Light 1---------------------------------------------------------
                    ...
                    if (StopLight == False):
                        ...
                        if isSecond == True:
                            ...
                            # --- New threshold logic ---
                            if green_ratio <= 0.0007 and green_pixels <= 500:
                ...
                if enableSteeringControl:
                # Get steering adjustment from lane detector
                    with lane_keeper.lock:
                        lane_steering = lane_keeper.steering_adjustment
    
                    # Combine with Stanley controller
                    stanley_steering = steeringController.update(p, th, v, dt)
                    delta = 0.80 * stanley_steering + 0.20 * lane_steering
                else:
                    delta = 0
                #endregion
            qcar.write(u, delta)
        ...
        # Before cameraInterfacingLab.stop_cameras()
